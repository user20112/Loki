...
namespace Sharky
{
    public class GameConnection
    {
        ...
        public async Task CreateGame(String mapName, Race PlayerRace, Race BotRace, int randomSeed = -1)
        {
            var createGame = new RequestCreateGame();
            createGame.Realtime = true;
            createGame.DisableFog = false;
            if (randomSeed >= 0)
            {
                createGame.RandomSeed = (uint)randomSeed;
            }
            ...
            var player1 = new PlayerSetup();
            player1.Race = PlayerRace;
            player1.Type = PlayerType.Participant;
            createGame.PlayerSetup.Add(player1);

            var player2 = new PlayerSetup();
            player2.Race = BotRace;
        ...
        public async Task<uint> JoinGame(Race race, bool IsHost)
        {
            var request = new Request
            {
                JoinGame = new RequestJoinGame
                {
                    ...
                    {
                        FeatureLayer = new SpatialCameraSetup { CropToPlayableArea = true, AllowCheatingLayers = false, MinimapResolution = new Size2DI { X = 16, Y = 16 }, Resolution = new Size2DI { X = 128, Y = 128 }, Width = 10 },
                        Raw = true,
                        Score = true,
                        ShowCloaked = true,
                    ...
                    SharedPort = 8175,
                    ServerPorts = new PortSet { GamePort = 8176, BasePort = 8177 },
                    ClientPorts = { new Collection<PortSet> { new PortSet { GamePort = 8171, BasePort = 8171 } } }
                }
            };
            var response = await Proxy.SendRequest(request);
