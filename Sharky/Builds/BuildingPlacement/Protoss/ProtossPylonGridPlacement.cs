...
namespace Sharky.Builds.BuildingPlacement
{
    public class ProtossPylonGridPlacement
    {
        ...
        private Point2D GetValidPoint(float x, float y, int baseHeight, Vector2 mineralLocationVector, float maxDistance, Point2D target)
        {
            var size = 2.25f;
        ...
        private Point2D GetValidPointInColumn(float x, int baseHeight, Vector2 mineralLocationVector, float yStart, float maxDistance, Point2D target)
        {
            var y = yStart;
            while (y - yStart < 30)
            {
